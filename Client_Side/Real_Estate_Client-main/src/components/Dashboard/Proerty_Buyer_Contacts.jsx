...
import { Confirm_Toffle, Delete_Agent_Property_Contact, Fetch_Agent_Contacts, Rejetced_Toggle } from '../../redux/action/propertyContactAction';
import { Button, IconButton, Tooltip } from '@mui/material';
import { AirlineStops, CheckCircleOutline, DeleteForever, DoNotDisturb } from '@mui/icons-material';
import Spinnerr from '../Spinnerr';
import { Link } from 'react-router-dom';

const Property_Buyer_Contacts = () => {
    const [side, setSide] = useState(false); // Sidebar state
    const {user,actionloading} = useSelector((state)=>state.User);
    const {property_buyers,contactloading,pccreateloading,confirmloading,rejectloading} = useSelector((state)=>state.PropertyContact);
    const dispatch = useDispatch();
    const [delid, setDelid] = useState("")
    const [itemid, setItemid] = useState("")

    useEffect(()=>{
        dispatch(Fetch_Agent_Contacts());
    },[dispatch])
    ...
    const handleconfirm = (id) => {
      setItemid(id);
      dispatch(Confirm_Toffle(id));
   }
    const handlerejected = (id) => {
      setItemid(id);
      dispatch(Rejetced_Toggle(id));
   }
    ...
    <div>
     ...
     <div className="dashboard-wrapper">
      ...
      <div className="dashboard-side min-h-screen ">
       ...
       <h2 className="text-xl md:text-2xl font-extrabold bg-black inline-block px-16 rounded-full text-white py-4">Proerty buyer list</h2>
       </div>

       <div className="wrapper">
   {contactloading ? (<div className='my-4 flex justify-center items-center h-[80vh]'><Spinnerr/></div>) : property_buyers?.length===0 ? (<div>
  ...
  <table className="min-w-full bg-white">
    ...
    <tbody className="whitespace-nowrap divide-y divide-gray-200">
      {property_buyers?.map((p,index)=>{
        ...
        <td className="p-4 text-sm">
          <Button variant='contained' sx={{background:"red"}} onClick={()=>handledelete(p._id)}>
            {(delid === p._id && pccreateloading) ? (<Spinnerr/>):(<DeleteForever/>)}
          </Button>
     {p.actions.complete===false && p.actions.rejected===false && (
          <>
           <Tooltip title="Completed">
           <IconButton onClick={()=>handleconfirm(p._id)}>
             {(confirmloading && itemid === p._id) ? (<Spinnerr/>):(<CheckCircleOutline />)}
           </IconButton>
         </Tooltip>
           <Tooltip title="Rejected">
           <IconButton onClick={()=>handlerejected(p._id)}>
             {(rejectloading && itemid === p._id) ? (<Spinnerr/>):(<DoNotDisturb />)}
           </IconButton>
         </Tooltip>
          </>
     )}
...
export default Property_Buyer_Contacts